...
- 2026-03-20: **한글과컴퓨터(HWP/HWPX) 처리 도구 추가** — `scripts/hwp_utils.py` 신규. HWP 읽기, HWPX 읽기·쓰기·치환 지원. **패미는 `pip install olefile` 설치 필수** (HWP 바이너리 읽기용). 사용법:
  ```
  python scripts/hwp_utils.py read 문서.hwp      # HWP 텍스트 추출
  python scripts/hwp_utils.py read 문서.hwpx     # HWPX 텍스트 추출
  python scripts/hwp_utils.py write 출력.hwpx "내용"  # HWPX 생성
  python scripts/hwp_utils.py replace 원본.hwpx 결과.hwpx "찾기" "바꾸기"
  ```
  Python 코드에서 import도 가능: `from scripts.hwp_utils import read_document, write_hwpx, replace_text_hwpx`
