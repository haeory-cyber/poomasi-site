...
**백업**: `index.html.bak_20260702`
- 메인 랜딩을 한지 버전으로 교체 (카피·5섹션 구조 불변, 에셋만 `landing-hanji.css/js`로 전환)
- 한지 바탕+수묵 성좌(노드 먹·감·쑥·단청 4계열), 히어로 가독성 페이드, 一~五 장 번호, 한지 카드, 도장풍 CTA
- 모바일 두손가락 핀치줌 신규(투명 터치막 z5, touch-action:pan-y) — 3회 실패했던 건 해결, `?debug=1` 진단 오버레이 내장
- 줌 버튼 CSS 선택자 버그 수정(기존 landing.css는 body 부착과 불일치로 버튼 미표시였음), 모바일에도 노출
- 프리뷰 경로 `preview-hanji.html` 병존(noindex)
